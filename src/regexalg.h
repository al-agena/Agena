...
static int OptLimit (lua_State *L, int pos) {
  if (lua_isnoneornil(L, pos)) return GSUB_UNLIMITED;
  if (lua_isfunction(L, pos))  return GSUB_CONDITIONAL;
...
static void check_subject (lua_State *L, int pos, TArgExec *argE) {
  ...
  if (stype != LUA_TSTRING && stype != LUA_TTABLE && stype != LUA_TUSERDATA) {
    ...
    if (lua_type(L, -1) != LUA_TFUNCTION)
      luaL_error(L, "Error: subject has no topointer method.");
    lua_pushvalue(L, pos);
    lua_call(L, 1, 1);
    type = lua_type(L, -1);
    if (type != LUA_TLIGHTUSERDATA)
      luaL_error(L, "Error: subject's topointer method returned %s (expected lightuserdata).",
                  lua_typename(L, type));
...
static void push_substrings (lua_State *L, TUserdata *ud, const char *text, TFreeList *freelist) {
  int i;
  if (lua_checkstack(L, ALG_NSUB(ud)) == 0) {
    if (freelist) freelist_free(freelist);
    luaL_error(L, "Error: cannot add %d stack slots.", ALG_NSUB(ud));
  }
  ...
  while ((argE.maxmatch < 0 || n_match < argE.maxmatch) && st <= (int)argE.textlen) {
    ...
    if (argE.reptype == LUA_TTABLE || argE.reptype == LUA_TFUNCTION) {
      if (lua_tostring(L, -1)) {
        ...
        freelist_free(&freelist);
        luaL_error(L, "Error: got invalid replacement value (a %s).", luaL_typename(L, -1));
      }
      if (argE.maxmatch != GSUB_CONDITIONAL)
        lua_pop(L, 1);
    }
...
static int algf_count (lua_State *L) {
  ...
  while (st <= (int)argE.textlen) {
    ...
    if (st < to) {
      ...
      st += ALG_CHARSIZE;  /* advance by 1 char (not replaced) */
    }
    else break;
  }
  /*------------------------------------------------------------------*/
...
static int finish_generic_find (lua_State *L, TUserdata *ud, TArgExec *argE, int method, int res) {
  if (ALG_ISMATCH(res)) {
