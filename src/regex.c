...
/* In Solaris configure pcre2-10.47 or higher like this:
   ./configure --disable-symvers  && make && make install */
...
static int generate_error (lua_State *L, const TPcre2 *ud, int errcode) {
  ...
  if (key)
    ...
    return luaL_error(L, "Error: PCRE2 code %d", errcode);
}
...
static int Lpcre2_maketables (lua_State *L) {
  *(const void**)lua_newuserdata(L, sizeof(void*)) = pcre2_maketables(NULL);
...
static int compile_regex (lua_State *L, const TArgComp *argC, TPcre2 **pud) {
  ...
  if (argC->locale) {
    ...
    if (NULL == setlocale(LC_CTYPE, argC->locale))  /* set new locale */
      return luaL_error(L, "Error: cannot set locale.");
    ud->tables = pcre2_maketables(NULL);  /* make tables with new locale */
  ...
  if (!ud->pr) {
    if (push_error_message(L, errcode))
      return luaL_error(L, "Error: %s (pattern offset: %d).", lua_tostring(L, -1), erroffset + 1);
    else
      return luaL_error(L, "Error: %s (pattern offset: %d).", "pattern compile error", erroffset + 1);
  }
  if (0 != pcre2_pattern_info(ud->pr, PCRE2_INFO_CAPTURECOUNT, &ud->ncapt))
    return luaL_error(L, "Error: could not get pattern info.");
...
void do_named_subpatterns (lua_State *L, TPcre2 *ud, const char *text) {
...
static int Lpcre2_dfa_exec (lua_State *L) {
  ...
  if (!wspace)
    luaL_error(L, "Error: malloc failed.");
...
static int Lpcre2_gc (lua_State *L) {
  ...
  lua_setmetatabletoobject(L, 1, NULL, 1);  /* addition 2.38.4 */
  return 0;
}
...
/* static void checkarg_create (lua_State *L, int idx, TArgComp *argC) {
  argC->pattern = luaL_checklstring(L, idx, &argC->patlen);
  argC->cflags = ALG_GETCFLAGS(L, idx + 1);
  ALG_GETCARGS(L, idx + 2, argC);
} */
...
/* static int algf_create (lua_State *L, int idx) {
  TArgComp argC;
  checkarg_create(L, idx, &argC);
  return compile_regex(L, &argC, NULL);
} */
...
static const luaL_Reg r_methods[] = {
  ...
  {"patterninfo", Lpcre2_pattern_info},
  {"fullinfo",    Lpcre2_pattern_info},
  {"__gc",        Lpcre2_gc},
...
static const luaL_Reg r_functions[] = {
  {"match",       algf_match},
  {"find",        algf_find},
  {"search",      algf_find},  /* just for testing purposes (stores for C functions) */
  {"gmatch",      algf_gmatch},
  {"gsub",        algf_gsub},
  {"count",       algf_count},
  {"split",       algf_split},
...
static void alg_register (lua_State *L, const luaL_Reg *r_methods,
  ...
  /* we need this in order for the metamethods to be properly assigned to objects */
  lua_pushvalue(L, -1);
...
REX_API int REX_OPENLIB (lua_State *L) {
  char buf_ver[64];
  /* register library */
  luaL_register(L, AGENA_REGEXLIBNAME, r_functions);
  /* check for correct PCRE2 library version */
  pcre2_config(PCRE2_CONFIG_VERSION, buf_ver);
  if (PCRE2_MAJOR > atoi(buf_ver)) {
    return luaL_error(L, "Error: %s requires at least version %d of PCRE2 library.",
  ...
  /* register storage table to `regex.find`, 3.5.3 */
  agnL_gettablefield(L, AGENA_REGEXLIBNAME, "find", NULL, 0);
  lua_createtable(L, 0, 16);
  agn_setstorage(L, -2);
  agn_poptop(L);  /* drop `strings.find` */
  return 1;
}
